...
- [ADR-002: Dual Format Design (Markdown + JSON-LD)](/docs/adr/adr-002-dual-format-design/) — **refined/amended by this ADR**: ADR-002 framed Markdown and JSON-LD as co-equal and bidirectionally convertible; ADR-011 designates Markdown canonical and JSON-LD a derived projection.
- [ADR-009: OKF Compliance as a Superset](/docs/adr/adr-009-okf-compliance-superset/) — the `.jsonld` extension keeps the projection outside OKF's `*.md` concept glob.
- [ADR-007: GitHub Raw URLs for Schema IDs](/docs/adr/adr-007-github-raw-urls-for-schema-ids/) — the derived JSON-LD projection carries the resolvable `@context`/schema `$id` URIs.
- [ADR-012: OKF Conformance as a Tested Invariant](/docs/adr/adr-012-okf-conformance-tested-invariant/) — makes the lossless round-trip a CI-enforced check rather than an assertion.
