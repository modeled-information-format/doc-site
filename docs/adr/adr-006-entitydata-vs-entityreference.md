...
- [ADR-004: Three-Tier Trait Inheritance](/docs/adr/adr-004-three-tier-trait-inheritance/) — the trait-inheritance model governs how an ontology composes the `entity_type` schema that EntityData's open fields conform to.
- [ADR-002: Dual-Format Design](/docs/adr/adr-002-dual-format-design/) — both entity representations carry through the Markdown and JSON-LD projections.
