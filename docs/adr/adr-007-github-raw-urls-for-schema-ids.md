...
- [ADR-002: Dual-Format Design](/docs/adr/adr-002-dual-format-design/) — the JSON-LD projection requires resolvable schema `$id` / `@context` URIs, which is the constraint this ADR satisfies.
- [ADR-011: Markdown-Canonical with Derived JSON-LD](/docs/adr/adr-011-markdown-canonical-derived-jsonld/) — the derived JSON-LD projection that carries the namespace IRI established here.
